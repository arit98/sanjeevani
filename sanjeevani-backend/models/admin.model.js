...
class Admin {
    ...
    async create(data, callback) {
        ...
        var sql = `INSERT INTO admin(role, email, password, created_at) VALUES (
        ...
        if (insertId) {
            var sql = `INSERT INTO admin(user_id,name, phone_number, created_at) VALUES (
    ...
    async show(id, callback) {
        ...
        if (id) {
            ...
            for (var item of userData) {
                ...
                // fetch user_details
                var sql1 = `SELECT * FROM admin WHERE user_id=${mysql.escape(item.id)}`;
                ...
                // fetch user_address
                var sql2 = `SELECT * FROM admin WHERE user_id=${mysql.escape(item.id)}`;
    ...
    getOne(data, callback) {
        var sql = `SELECT * FROM admin WHERE ${data.column}=${mysql.escape(data.value)}`;
        conn.query(sql, (err, result) => {
            if (err) throw err;
            if (result.length > 0) {
    ...
    updateUser(data, id, callback) {
        var sql = `UPDATE admin set updated_at=NOW(),? WHERE id=?`;
