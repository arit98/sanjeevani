...
class NormalUsers {
    async create(data, callback) {
        ...
        var sql = `INSERT INTO normaluser(role, email, password, created_at) VALUES (
    ...
    async show(id, callback) {
        ...
        if (id) {
            var sql = `SELECT * FROM normaluser WHERE id=${mysql.escape(id)}`;
            ...
            var sql = `SELECT * FROM normaluser ORDER BY id DESC`;
    ...
    getOne(data, callback) {
        var sql = `SELECT * FROM normaluser WHERE ${data.column}=${mysql.escape(data.value)}`;
        conn.query(sql, (err, result) => {
            if (err) throw err;
            if (result.length > 0) {
    ...
    updateUser(data, id, callback) {
        var sql = `UPDATE normaluser set updated_at=NOW(),? WHERE id=?`;
    ...
    deleteUser(id, callback) {
        var sql = `DELETE normaluser from sanjeevani WHERE id=?`;
...
module.exports = new NormalUsers();
