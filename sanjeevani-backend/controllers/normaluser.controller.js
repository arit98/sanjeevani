var normaluserModel = require("../models/normaluser.model");
...
exports.registration = async (req, res) => {
  ...
  if (role && email && password1 && password2 && name && phone_number) {
    ...
    var existUser = await new Promise((resolve, rejects) => {
      normaluserModel.getOne(data, (err, result) => {
        if (result == null) {
          resolve(true);
        } else {
          resolve(false);
    ...
    if (existUser) {
      if (password1 === password2) {
        ...
        normaluserModel.create(userData, (err, result) => {
          if (err) {
            res.status(200).json({
              status: false,
...
exports.login = (req, res) => {
  ...
  if (email && password) {
    ...
    normaluserModel.getOne(data, async (err, result) => {
      if (!err) {
    ...
    normaluserModel.getOne(data, async (err, result) => {
      if (!err) {
        if (password1 === password2) {
          ...
          normaluserModel.updateUser(userData, result.id, (err, result) => {
            if (err) {
              res.status(200).json({
                status: false,
...
exports.getUser = (req, res) => {
  ...
  if (id) {
    normaluserModel.show(id, (err, result) => {
      if (err) {
        res.status(200).json({
          status: false,
          msg: " data not found",
...
exports.getAllUser = (req, res) => {
  normaluserModel.show(null, (err, result) => {
    if (err) {
      res.status(200).json({
        status: false,
        msg: " data not found",
...
exports.updateUserDetails = async (req, res) => {
  ...
  if (req.files.image_profile != undefined) {
    ...
    var image_profile = await new Promise((resolve, reject) => {
      normaluserModel.getUserdetails(user_id, (err, result) => {
        if (err) {
          resolve(null);
        } else {
  ...
  if (user_id) {
    normaluserModel.updateUserDetails(data, user_id, (err, result) => {
      if (err) {
        res.status(200).json({
          status: false,
...
exports.statusChange = (req, res) => {
  ...
  if (user_id && approve_status) {
    ...
    normaluserModel.updateUser(data, user_id, (err, result) => {
      if (err) {
        res.status(200).json({
          status: false,
...
exports.deleteUser = (req, res) => {
  normaluserModel.deleteUser(null, (err, result) => {
    if (err) {
      res.status(200).json({
        status: false,
        msg: " data not found",
