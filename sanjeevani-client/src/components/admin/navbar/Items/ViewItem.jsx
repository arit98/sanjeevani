...
const ViewItem = () => {
  ...
  const loadData = async () => {
    const response = await MedicineServices.getAll();
  ...
  return (
    <div className="py-10 overflow-x-auto w-4/5 right-0 absolute">
      <div className="flex items-start -mt-16 mr-20">
        <div className="mx-auto mr-6 mt-4 container bg-white shadow rounded w-[1450px]">
                  ...
                  ? data.map((item) => (
                      <tr className="h-24 border-gray-300 border-b">
                        <td className="text-sm pr-6 whitespace-no-wrap text-gray-800 tracking-normal leading-4">
                          <div className="relative w-10 text-gray-600"></div>
                        </td>
                        <td className="text-sm pr-6 whitespace-no-wrap text-gray-800 tracking-normal leading-4">
                          {item.id}
                        </td>
                        ...
                        <td className="text-sm pr-6 whitespace-no-wrap text-gray-800 tracking-normal leading-4">
                          {item.category_id===1?"Gastric":item.category_id===2?"Daily Use":item.category_id===3?"Antibiotics":item.category_id===4?"Syrup":item.category_id===5?"Vitamin":item.category_id===6?"Baby Care":item.category_id===7?"Gynocology":item.category_id===8?"Sugar":"Pain Killer"}
                        </td>
                        <td className="text-sm pr-6 whitespace-no-wrap text-gray-800 tracking-normal leading-4">
                          {item.company_id}
                        </td>
                        ...
                        <td className="pr-8 relative">
                            <button className="h-10 w-32 text-white rounded-sm" onClick={activeDeactive}>
                              {item.status===1?(<span className="bg-lime-500 p-2">Active</span>):item.status===2?(<span className="bg-red-500 p-2">Deactive</span>):(<span className="bg-blue-500 p-2">Out Of Stock</span>)}
                            </button>
                        </td>
                        <td className="text-sm pr-6 whitespace-no-wrap text-gray-800 tracking-normal leading-4">
                          {item.created_at}
